...
import json
import requests
import urllib#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  7 11:42:36 2019
...
def MoveFiles():
    Source = '/home/polo/.config/spyder-py3/PhD/PhD_Dataset'
    Destination = '/home/polo/.config/spyder-py3/PhD/Tourism30'

    Galeries_Matrix = np.array(galeries).reshape(len(Countries),10)
    i = 0
    for Country in Countries:
        for j in range (10):
            S = Source+'/'+Country+'/'+Galeries_Matrix[i,j]+'/'+Galeries_Matrix[i,j]+'.json'
            D = Destination+'/'+Country+'/'+Galeries_Matrix[i,j]+'/'+Galeries_Matrix[i,j]+'.json'
            shutil.move(S,D)
        i += 1
